...
data_frame_parser_logger = Logger().get_logger("data_frame_parser_logger")
...
class DataFrameParser(ExchangeAPI, DataBuilder):
    ...
    def __init__(self, name, password, x_application_id):
        ...
        try:
            ...
            data_frame_parser_logger.exception(
    ...
    async def second_cycle(self) -> None:
        data_frame_parser_logger.info(
        ...
        tasks = []
        data_frame_parser_logger.info(
            'Starting create coroutines with asyncio',
        )
        ...
        data_frame_parser_logger.info(
            f"Processed in {chronometer(start)}")

    def third_cycle(self) -> None:
        data_frame_parser_logger.info("Third cycle")
        start = time.time()
        ...
        def set_runner_name(market_id: str, selection_id: Any) -> str:
            ...
            filter_by_mk_id: List[Dict[str, Any]] = list(
                filter(
                    ...
                    self.temporary_runners_list
                )
            )
            print(f"Getting runner names {chronometer(start)}", end="\r")
            if len(filter_by_mk_id) >= 1:
                index_to_access_list = self.temporary_runners_list.index(
                    filter_by_mk_id[0])
                ...
                if len(filter_by_selection_id) > 0:
                    index_to_access_runner = self.temporary_runners_list[index_to_access_list]["runners"].index(
                        filter_by_selection_id[0],
                    )
                    name += filter_by_selection_id[0]["runner_name"]
                    handicap = filter_by_selection_id[0].get("handicap", 0)
                    if handicap != 0:
                        name += f"~^~{handicap}"
        ...
        self.temporary_runners_list = self.runners_list
    ...
    def to_csv(self):
        if self.df is not None:
            ...
            data_frame_parser_logger.info(f"Saved as {outputname}")
        else:
            data_frame_parser_logger.info(
    ...
    def load_from_csv(self, file_name: str) -> pd.DataFrame:
        ...
        try:
            ...
            data_frame_parser_logger.exception(f"{ex}")
